...
class Config(object):
    ...
    def __init__(self, use_words, dataset):
        ...
        dic = '+word/' if use_words else '+character/'
        self.save_path = dataset + '/saved_dict/classes/' + self.model_name + dic + self.model_name + '.pth'        # 模型训练结果
        self.save_dic = dataset + '/saved_dict/classes/' + self.model_name + dic + self.model_name + '_'      # 模型训练结果
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        bert_config = BertConfig(vocab_size_or_config_json_file=self.vocab_size,
                    hidden_size=config.hidden_size,
                    layer_norm_eps=1e-5, hidden_act='relu', 
                    max_position_embeddings=config.pad_size)
