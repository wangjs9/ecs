...
class Decoder(nn.Module):
    def __init__(self, vocab_size, embed_size, enc_hidden_size, dec_hidden_size, dec_layer, embed, dropout=0.5):
        ...
        self.rnn = nn.GRU(embed_size, enc_hidden_size, dec_layer, batch_first=True)
        self.out = nn.Linear(dec_hidden_size, vocab_size)
        self.grammar_filter = Filter(dec_hidden_size)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, ctx, ctx_lengths, tgt, tgt_lengths, hid, grammar):
        ...
        output = self.grammar_filter(output, hid, grammar)
